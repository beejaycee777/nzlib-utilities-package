...
namespace NZLib.Utilities.LanguageCore
{
    ...
    public class LanguageController
    {
        ...
        private LanguageController ()
        {
            ...
            {
                // Necessary to jump over .meta files which have the same name as .xml and are considered as TextAssets too.
                var t_read = true;
                for (var l = 0; l < _languages.Count; ++l)
                {
                    if (_languages[l].Key == t_files[i].name)
                    {
                        t_read = false;
                    }
                }
        ...
        private string SystemLanguageToString ()
        {
            ...
            var t_default = "en";            
            if (Application.platform != RuntimePlatform.Android) // Android cannot access 'StreamingAssets' folder.
            {
